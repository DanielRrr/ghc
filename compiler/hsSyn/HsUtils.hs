...
userHsLTyVarBndrs loc bndrs braces = (L loc . uncurry id) <$> (zip [ UserTyVar noExt v | v <- bndrs] braces)

userHsTyVarBndrs :: SrcSpan -> [IdP (GhcPass p)] -> [TvbBraces] -> [LHsTyVarBndr (GhcPass p)]
-- Caller sets location
userHsTyVarBndrs loc bndrs braces = (L loc . uncurry id) <$> (zip [ UserTyVar noExt (L loc v)
                             | v <- bndrs] braces)
